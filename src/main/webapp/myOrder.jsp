...
<%
    // Get all order items passed from the Servlet (MyOrderServlet)
    List<CartModel> allItems = (List<CartModel>) request.getAttribute("orderItems");

    // Group items by deliveryId to show separate order cards
    Map<Integer, List<CartModel>> groupedOrders = new LinkedHashMap<>();
    if (allItems != null) {
        groupedOrders = allItems.stream().collect(
            Collectors.groupingBy(CartModel::getDeliveryId, LinkedHashMap::new, Collectors.toList())
        );
    }
%>
...
<div class="max-w-6xl mx-auto px-4 py-12">
  ...
  <%-- Show "No Orders Yet" message if no orderItems found --%>
  ...
  <% } else { %>
    ...
    <%-- Loop through each deliveryId group (one group = one order) --%>
    <% for (Map.Entry<Integer, List<CartModel>> entry : groupedOrders.entrySet()) {
         ...
         double total = 0;
         String status = orderItems.get(0).getStatus(); // Assuming all items in the same order have same status
    %>
    ...
    <div class="bg-white rounded-xl shadow-md p-8 mb-8 border border-black">
      ...
      <table class="w-full text-left border border-gray-200 rounded-lg overflow-hidden mb-4 shadow-sm">
        ...
        <tbody class="text-gray-800 bg-white">
          ...
          <%-- Loop through each CartModel item inside one order --%>
          <% for (CartModel item : orderItems) {
               double itemTotal = item.getPrice() * item.getQuantity(); // Calculate line total
               total += itemTotal;
          %>
          <tr class="border-b hover:bg-orange-50 transition duration-200">
            <td class="p-4">
              <div class="flex items-center gap-4">
                <!-- Encapsulation: using CartModel getters -->
      ...
      <!-- Buttons to cancel or download receipt -->
...
<!-- JS confirmation for cancel button -->
<script>
